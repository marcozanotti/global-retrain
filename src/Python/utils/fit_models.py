...
def retrain_sf_model(
    ...
    for ts in series:
        ...
        for i in fitting_ids:
        
            module_logger.info(f'Step {int(i / retrain_window + 1)} of {n_fitting}')
            ...
            if intervals is None:
                # fit_ts_tmp = engine.fit(df = train_df_tmp)
                engine.fit(df = train_df_tmp)
            else:
            ...
            out_sample_df_tmp = engine.predict(h = horizon_sf, level = levels)
            ...
            n_samples = test_window - horizon
            for s in range(retrain_window):
                s_id = i + s
                if s_id > n_samples:
                    break
                module_logger.info(f'Saving sample {s_id} of {n_samples}...')
                out_sample_s = out_sample_df_tmp.head(horizon + s).tail(horizon)
                out_sample_s.insert(0, 'sample', s_id)
                if ts == series[0]:
                    save_data(
                        data = out_sample_s, 
                        path_list = ['results', dataset_name, frequency, model_name, retrain_window, 'outsample', 'tmp'],
                        name_list = [dataset_name, frequency, model_name, retrain_window, 'outsample', s_id],
                        ext = ext,
...
def retrain_model(config):
    ...
    for m in model_names:
        ...
        for rs in retrain_scenarios:
            ...
            # this is necessary for SF models to avoid issues with previous fittings
