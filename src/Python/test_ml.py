...
## UNIONE DI TUTTI I PARQUET
out_sample_files = [i for i in os.listdir('results') if 'parquet' in i and 'out_sample' in i]
time_df_files = [i for i in os.listdir('results') if 'parquet' in i and 'time_df' in i]
in_sample_files = [i for i in os.listdir('results') if 'parquet' in i and 'in_sample' in i]

if out_sample_files:
    schema = pq.ParquetFile('results/'+out_sample_files[0]).schema_arrow
    with pq.ParquetWriter("results/final_out_output.parquet", schema=schema) as writer:
        for file in out_sample_files:
            writer.write_table(pq.read_table('results/'+file, schema=schema))
if time_df_files:
    schema = pq.ParquetFile('results/'+time_df_files[0]).schema_arrow
    with pq.ParquetWriter("results/final_time_output.parquet", schema=schema) as writer:
        for file in out_sample_files:
            writer.write_table(pq.read_table('results/'+file, schema=schema))
if in_sample_files:
    schema = pq.ParquetFile('results/'+in_sample_files[0]).schema_arrow
    with pq.ParquetWriter("results/final_in_output.parquet", schema=schema) as writer:
        for file in out_sample_files:
            writer.write_table(pq.read_table('results/'+file, schema=schema))